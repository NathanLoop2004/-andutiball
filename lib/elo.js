...
// Lo que se le manda a la página: solo nombre → elo/división, para anunciarlo en el chat
function paraLaSala(tabla) {
  const salida = {};
  for (const f of Object.values(tabla)) {
    const d = divisionDe(f.elo);
    salida[f.nombre.toLowerCase()] = { elo: f.elo, partidos: f.partidos, division: d.nombre, emoji: d.emoji };
  }
  return salida;
}
